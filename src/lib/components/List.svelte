...
<div class="flex flex-col gap-2">
  {#each questions.slice(page * size, size * (page + 1)) as question}
    <a href="/question/{question.id}" class="p-4 bg-light-surface-300/50 dark:bg-[#262b29] rounded-lg">
      ...
      <span class="text-sm">Enviado em {DateTime.fromISO(question.sentAt).setLocale('pt-BR').toFormat('EEEE, dd/LL/yyyy')}</span>
    </a>
  {/each}
</div>

<span class="text-center w-full text-sm">Exibindo {questions.slice(page * size, size * (page + 1)).length} de <Select size="sm" class="w-16 inline" placeholder="Número de itens por página..." items={sizes} bind:value={size} onchange={() => page = 0}/> perguntas por página</span>