...
{#if emoji.startsWith('$')}
  <div class="aspect-square h-[2.2rem]">
    <img src={emoji.match(/\$\[Imagem\]\((.*?)\)\$/)[1]} class="h-[2.2rem] mx-auto" alt="Emoji com imagem">
  </div>
{:else if emoji == '##'}
  <svg width="2.2rem" height="2.2rem" class="fill-light-surface-950 dark:fill-white" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
...
{:else}
  <div class="w-[2.2rem] h-[2.2rem]" use:stemoji={emoji}>{emoji}</div>
{/if}